...
import { keyPaths } from './assets.js';
import {
  CAPABILITIES,
  COPYRIGHT_YEAR,
  LINKS,
...
function chart() {
  ...
  const top = 28;
  const base = 172;
  const { mid, starts, strokes, area } = keyPaths(x0, x1, top, base);
  const guides = [0.3, 0.55, 0.8]
    .map((f) => {
      const x = r(x0 + (x1 - x0) * f);
      return `<line x1="${x}" y1="${top - 14}" x2="${x}" y2="${base + 14}" stroke="var(--border)" stroke-dasharray="2 6"/>`;
    })
    .join('');

  return `<div class="chart reveal">
          <svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Three ways in — a passphrase, a recovery phrase and a passkey — converging on one root key that never leaves the device">
            <title>Three paths, one root key</title>
            <defs>
              <!--
                User space, not the default bounding box: the middle path is a
                straight horizontal line, and a box with no height would leave
                a bounding-box gradient with nothing to paint into.
              -->
              <linearGradient id="key-stroke" gradientUnits="userSpaceOnUse" x1="${x0}" y1="0" x2="${x1}" y2="0">
                <stop offset="0%" stop-color="var(--accent-bright)"/>
                <stop offset="100%" stop-color="var(--violet)"/>
              </linearGradient>
              <linearGradient id="key-area" gradientUnits="userSpaceOnUse" x1="${x0}" y1="0" x2="${x1}" y2="0">
                <stop offset="0%" stop-color="var(--accent-bright)" stop-opacity="0.3"/>
                <stop offset="100%" stop-color="var(--accent-bright)" stop-opacity="0"/>
              </linearGradient>
            </defs>
            ${guides}
            <path class="chart__area" d="${area}" fill="url(#key-area)"/>
            ${strokes
              .map(
                (d) =>
                  `<path class="chart__curve" d="${d}" pathLength="1" fill="none" stroke="url(#key-stroke)" stroke-width="2.5" stroke-linecap="round"/>`,
              )
              .join('\n            ')}
            ${starts
              .map((y) => `<circle cx="${x0}" cy="${y}" r="4.5" fill="var(--accent-bright)"/>`)
              .join('\n            ')}
            <g class="chart__tip">
              <circle cx="${x1}" cy="${mid}" r="11" fill="var(--violet)" opacity="0.16"/>
              <circle cx="${x1}" cy="${mid}" r="4.5" fill="var(--violet)"/>
            </g>
          </svg>
          <p class="chart__foot"><span>Passphrase · phrase · passkey</span><span>One root key</span></p>
        </div>`;
}
  ...
  const groups = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          icon: ICONS.x,
          label: `${PROFILE.handle} on X`,
          hint: 'Say hello',
          href: LINKS.x,
          external: true,
          terms: `x twitter ${PROFILE.handle} contact dm social`,
        },
        {
          icon: ICONS.repo,
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          hint: PROFILE.handle,
          action: 'copy-handle',
          terms: `copy handle username ${PROFILE.handle} clipboard`,
        },
        {
          icon: ICONS.copy,
