...
/**
 * The three routes to one key — a passphrase, a recovery phrase and a passkey
 * — drawn as curves that start apart on the left and converge on a single
 * point at the right.
 *
 * Returns the strokes, the wedge they enclose and the y they meet at, so a
 * caller can fill under them and place the node without redoing the geometry.
 * Each stroke carries `pathLength="1"` at the call site, which lets the draw-in
 * animation use normalised dash units.
 */
export function keyPaths(x0, x1, top, base) {
  const mid = r((top + base) / 2);
  const cx = r(x0 + (x1 - x0) * 0.62);
  const from = (y) => `M${r(x0)},${r(y)}C${cx},${r(y)} ${cx},${mid} ${r(x1)},${mid}`;

  return {
    mid,
    starts: [r(top), mid, r(base)],
    strokes: [from(top), from(mid), from(base)],
    // Out along the top curve, back along the bottom one, then closed.
    area: `${from(top)}C${cx},${mid} ${cx},${r(base)} ${r(x0)},${r(base)}Z`,
  };
}
...
export function hero(options = {}) {
  ...
  const plotTop = 118;
  const plotBase = 246;
  const { mid: plotMid, starts, strokes, area } = keyPaths(plotX0, plotX1, plotTop, plotBase);
  const guides = [0.3, 0.55, 0.8]
    .map((f) => {
      const x = r(plotX0 + (plotX1 - plotX0) * f);
      return `<line class="${ns}hair" x1="${x}" y1="${plotTop - 14}" x2="${x}" y2="${plotBase + 14}" stroke-width="1" stroke-dasharray="2 6" opacity="0.7"/>`;
    })
    .join('\n    ');

  const label = `${name} — solo builder shipping zero-knowledge, offline-first web apps`;
  ...
  <defs>
    ...
    <!--
      User space, not the default bounding box: the middle path is a straight
      horizontal line, and a box with no height would leave a bounding-box
      gradient with nothing to paint into.
    -->
    <linearGradient id="h-conv" gradientUnits="userSpaceOnUse" x1="${plotX0}" y1="0" x2="${plotX1}" y2="0">
      <stop class="${ns}sa" offset="0%"/>
      <stop class="${ns}sb" offset="100%"/>
    </linearGradient>
    <linearGradient id="h-area" x1="0" y1="0" x2="1" y2="0">
  ...
  <g class="h-mono">
    <text class="${ns}fa" x="${plotX0}" y="90" font-size="12" letter-spacing="2.8">PASSPHRASE  ·  PHRASE  ·  PASSKEY</text>
    ${guides}
    <path class="h-fill" d="${area}" fill="url(#h-area)"/>
    ${strokes
      .map(
        (d) =>
          `<path class="h-curve" d="${d}" pathLength="1" fill="none" stroke="url(#h-conv)" stroke-width="2.5" stroke-linecap="round"/>`,
      )
      .join('\n    ')}
    ${starts
      .map((y) => `<circle class="${ns}aa" cx="${plotX0}" cy="${y}" r="4.5"/>`)
      .join('\n    ')}
    <g class="h-tip">
      <circle class="${ns}ab" cx="${plotX1}" cy="${plotMid}" r="11" opacity="0.16"/>
      <circle class="${ns}ab" cx="${plotX1}" cy="${plotMid}" r="4.5"/>
    </g>
    <text class="${ns}fa" x="${plotX1}" y="${plotBase + 26}" font-size="12" letter-spacing="1.4" text-anchor="end">one root key  ·  never uploaded</text>
  </g>
</svg>
`;
}
...
export function ogCard() {
  ...
  const H = 630;
  const t = THEMES.dark;
  // The band sits clear of the tagline above it and the rule below it.
  const { mid, starts, strokes, area } = keyPaths(96, 1104, 408, 528);
  ...
  <defs>
    ...
    <!-- User space so the straight middle path still has a gradient to paint. -->
    <linearGradient id="o-conv" gradientUnits="userSpaceOnUse" x1="96" y1="0" x2="1104" y2="0">
      <stop offset="0%" stop-color="${t.accentA}"/>
      <stop offset="100%" stop-color="${t.accentB}"/>
    </linearGradient>
    <linearGradient id="o-area" x1="0" y1="0" x2="1" y2="0">
  ...
  <g font-family="${FONT_MONO}">
    ...
    <text x="96" y="378" font-size="27" fill="${t.text}">WebCrypto  ·  Next.js  ·  Passkeys  ·  Offline-first</text>
  </g>
  <path d="${area}" fill="url(#o-area)"/>
  ${strokes
    .map((d) => `<path d="${d}" fill="none" stroke="url(#o-conv)" stroke-width="4" stroke-linecap="round"/>`)
    .join('\n  ')}
  ${starts.map((y) => `<circle cx="96" cy="${y}" r="7" fill="${t.accentA}"/>`).join('\n  ')}
  <circle cx="1104" cy="${mid}" r="9" fill="${t.accentB}"/>
